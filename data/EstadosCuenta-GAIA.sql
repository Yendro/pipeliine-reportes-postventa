--RAUL MORENO  12/11/2025

-- CREATE OR REPLACE VIEW `Ventas.Estado_de_Cuentas` AS        ----PARA VOLVER A CREAR LA VISTA SE DESCOMENTA ESTA LINEA 
WITH Ventas3Version AS (
    ...
    --CUSTO
        SELECT
            ...
            NombreCopro,  ApellidoPaternoCopo,  ApellidoMaternoCopo,
            telefono_celular,
            correo_electronico, 
            cuentaBeneficiario, 
            nombre_status, 
        ...
        FROM EXTERNAL_QUERY("terraviva-439415.us.custo",
          """SELECT 
                  v.id_venta, 
                    ...
                    cop.nombre AS NombreCopro, 
                    cop.apellido_p AS ApellidoPaternoCopo, 
                    cop.apellido_m AS ApellidoMaternoCopo,
           ...
           LEFT JOIN copropietario AS cop ON v.id_venta = cop.id_venta
           ...
           WHERE v.status_venta IN (7, 8)""")
            GROUP BY id_venta, Origen, nombre_desarrollo, privada, numero_unidad,  NombreAsesor, ApellidoPaternoAsesor, ApellidoMaternoAsesor, NombreCliente, ApellidoPCliente, ApellidoMCliente,  NombreCopro,  ApellidoPaternoCopo,  ApellidoMaternoCopo, telefono_celular, correo_electronico,  cuentaBeneficiario,  nombre_status,  referencia_banco, Fecha_Contrato, Aprobado_juridico, precio_venta, dia_pago, total_pagado, cantidad_enganche, saldo_total, numero_pagos, Mensualidad, primera_fecha_vencido, saldo_vencido_total, total_requerido, pago_vencido_total, numero_pago, fecha_ultimo_ingreso, monto_ultimo_ingreso, monto_ultimo_ingreso_cobrado, siguiente_fecha_pago, fecha_promesa
    UNION ALL
    ...
    --DAM
        ...
        FROM EXTERNAL_QUERY("terraviva-439415.us.dam",
          """SELECT 
           ...
           LEFT JOIN (SELECT id_venta, cuentaBeneficiario FROM stp_bitacora WHERE status = 1 GROUP BY id_venta) AS stp ON v.id_venta = stp.id_venta
           LEFT JOIN usuario AS u ON v.id_usuario = u.id_usuario 
           LEFT JOIN copropietario AS cop ON v.id_cliente = cop.id_venta
           LEFT JOIN cliente AS c ON v.id_cliente = c.id_cliente
           LEFT JOIN unidades AS uni ON v.id_unidad = uni.id_unidad LEFT JOIN desarrollo AS d ON uni.id_desarrollo = d.id_desarrollo
           LEFT JOIN status_venta AS sv ON v.status_venta = sv.id_status
           LEFT JOIN (
    ...
    --Almaviva
        ...
        FROM EXTERNAL_QUERY("terraviva-439415.us.bq_almaviva",
          """SELECT 
           ...
           WHERE v.status_venta IN (7, 8)""") -- WHERE id_venta = 569
            GROUP BY id_venta, Origen, nombre_desarrollo, privada, numero_unidad,  NombreAsesor, ApellidoPaternoAsesor, ApellidoMaternoAsesor, NombreCliente, ApellidoPCliente, ApellidoMCliente,  NombreCopro,  ApellidoPaternoCopo,  ApellidoMaternoCopo, telefono_celular, correo_electronico,  cuentaBeneficiario,  nombre_status,  referencia_banco, Fecha_Contrato, Aprobado_juridico, precio_venta, dia_pago, total_pagado, cantidad_enganche, saldo_total, numero_pagos, Mensualidad, primera_fecha_vencido, saldo_vencido_total, total_requerido, pago_vencido_total, numero_pago, fecha_ultimo_ingreso, monto_ultimo_ingreso, monto_ultimo_ingreso_cobrado, siguiente_fecha_pago, fecha_promesa
    ...
    VentasFinal AS ( --LIMPIEZA DE DATOS
        SELECT
            v.id_venta,
            CONCAT(v.Desarrollo, ' ', v.Unidad) AS id,
            va.Marca,
            va.Desarrollo,
            ...
            TRIM(
                CONCAT(
                    INITCAP(TRIM(v.NombreCopro)), ' ', 
                    INITCAP(TRIM(v.ApellidoPaternoCopo)), ' ', 
                    INITCAP(TRIM(v.ApellidoMaternoCopo))
            )) AS Copropietario, 
            v.telefono_celular,
            ...
            -- (pago_vencido_total - saldo_vencido_total) AS Cobrado, SI UN CLIENTE PAGA DE MAS UN MES, SE REFLEJA LOS QUE TIENE INGRESADO HASTA AHORA 
    ...
    SELECT 
            ...
            REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(Copropietario, 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n') AS Copropietario,
            Asesor,
            nv.Sucursal,
            nv.Tipo,
            nv.Equipo,
    ...
    -- WHERE Cliente NOT IN ('Oscar Manuel Baas Baas', 'Demo2024 Demo Villa', 'Oficina Dam') 
    -- AND Desarrollo NOT IN ('Demo','DEMO', 'Vista Esmeralda', 'Real Del Angel') 
    -- AND Cliente LIKE '%Rubi Magali Arreola Jimenez%'
...
-- WHERE  Marca = "Almaviva"
-- AND Unidad = "L-177"
-- AND Cliente LIKE "%Martha Delfina%"

-- WHERE Equipo LIKE '%Dali%'
-- WHERE Estatus = 'Finalizado y Liquidado'  AND Acumado_Vencido != 0 --CASOS A REVISAR
GROUP BY id_venta, id, Marca, Desarrollo, Privada,   Unidad, Cliente, Copropietario, Asesor, Sucursal, Tipo, Equipo, telefono_celular, correo_electronico,  CuanetaBeneficiarioReal,  Estatus, Fecha_Contrato, Fecha_Firma_Contrato, precio_venta,   dia_pago, Total_cobrado, Enganche_pagado, total_por_cobrar, Meses_Financia, Mensualidad, Total_Requerido, Dias_Atrasado, fecha_ultimo_ingreso, Cobrado, Acumado_Vencido, numero_pago, Fecha_Proximo_pago, monto_ultimo_ingreso, fecha_promesa
-- COIN PARQURE PIMIENTA
-- PAGO Y ADEDUDO, RFC, NOMBRE, CRORREO
-- WHERE Cliente = 'Ernesto Galindo Obregon'
-- WHERE Cliente = 'Marco Antonio Cen Cen' AND Unidad ='L-005'
-- WHERE Cliente = 'Rodrigo Rosales Carrasco'